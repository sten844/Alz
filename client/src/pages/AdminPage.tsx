      ...
      <div className="min-h-screen flex flex-col bg-background">
        ...
        <main className="flex-1 flex items-center justify-center">
          <div className="text-center max-w-md mx-auto px-4">
            ...
            <h1 className="text-3xl mb-4 text-foreground" style={{ fontFamily: "'DM Serif Display', serif" }}>
              {t("Logga in", "Log in")}
            </h1>
        ...
        <div className="container py-8 max-w-5xl mx-auto">
          ...
          {activeTab === "articles" && (
            <>
              ...
              {showArticleForm && (
                <div className="bg-card rounded-2xl border border-border/50 p-6 mb-8 shadow-sm">
                  ...
                  <div className="space-y-4">
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t("Titel", "Title")}</label>
                    ...
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t("Sammanfattning", "Excerpt")}</label>
                      <textarea value={articleForm.excerpt} onChange={(e) => setArticleForm({ ...articleForm, excerpt: e.target.value })}
                        rows={2} className="w-full px-4 py-2.5 rounded-lg bg-background border border-border/50 text-sm focus:outline-none focus:ring-2 focus:ring-[#c05746]/30 resize-none"
                        placeholder={t("Kort sammanfattning...", "Short excerpt...")} />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t("Innehåll (Markdown)", "Content (Markdown)")}</label>
                      <textarea value={articleForm.content} onChange={(e) => setArticleForm({ ...articleForm, content: e.target.value })}
                        rows={12} className="w-full px-4 py-2.5 rounded-lg bg-background border border-border/50 text-sm focus:outline-none focus:ring-2 focus:ring-[#c05746]/30 font-mono resize-y"
                        placeholder={t("Skriv artikelns innehåll i Markdown...", "Write article content in Markdown...")} />
                    </div>
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-1">{t("Kategori", "Category")}</label>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-1">{t("Språk", "Language")}</label>
                      ...
                      <div>
                        <label className="block text-sm font-medium text-foreground mb-1">{t("Publiceringsdatum", "Publish date")}</label>
                        <input type="datetime-local" value={articleForm.publishedAt} onChange={(e) => setArticleForm({ ...articleForm, publishedAt: e.target.value })}
                          className="w-full px-4 py-2.5 rounded-lg bg-background border border-border/50 text-sm focus:outline-none focus:ring-2 focus:ring-[#c05746]/30" />
                      </div>
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">{t("Bild-URL (valfritt)", "Image URL (optional)")}</label>
                    ...
                    <div className="flex items-center justify-between pt-2">
                      <label className="flex items-center gap-2 cursor-pointer">
                        ...
                        <span className="text-sm text-foreground">{t("Publicerad", "Published")}</span>
                      </label>
                      <button onClick={handleSaveArticle} disabled={isArticleSaving}
                        ...
                        {isArticleSaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                        {t("Spara", "Save")}
                      </button>
                    </div>
                  </div>
              ...
              {/* Articles list */}
              {articlesLoading ? (
                <div className="flex justify-center py-16"><Loader2 className="w-8 h-8 animate-spin text-[#c05746]" /></div>
              ) : (
                <div className="space-y-3">
                  {allArticles && allArticles.length > 0 ? (
                    allArticles.map((article) => (
                      <div key={article.id} className="bg-card rounded-xl border border-border/50 p-4 flex items-center gap-4 hover:shadow-sm transition-shadow">
                        {article.imageUrl && <img src={article.imageUrl} alt="" className="w-16 h-16 rounded-lg object-cover shrink-0" />}
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xs px-2 py-0.5 rounded-full bg-accent text-muted-foreground">{article.category}</span>
                            <span className="text-xs text-muted-foreground">{article.language.toUpperCase()}</span>
                            {!article.published && <span className="text-xs px-2 py-0.5 rounded-full bg-yellow-100 text-yellow-700">{t("Utkast", "Draft")}</span>}
                          </div>
                          <h3 className="font-semibold text-foreground truncate">{article.title}</h3>
                          <p className="text-xs text-muted-foreground">{new Date(article.publishedAt).toLocaleDateString("sv-SE")}</p>
                        </div>
                        <div className="flex items-center gap-1 shrink-0">
                          <button onClick={() => handleEditArticle(article)} className="p-2 rounded-full hover:bg-accent transition-colors" title={t("Redigera", "Edit")}>
                            <Pencil className="w-4 h-4 text-muted-foreground" />
                          </button>
                          {deleteArticleConfirm === article.id ? (
                            <div className="flex items-center gap-1">
                              <button onClick={() => deleteArticleMutation.mutate({ id: article.id })} className="px-3 py-1 bg-red-500 text-white rounded-full text-xs font-medium hover:bg-red-600 transition-colors">
                                {t("Bekräfta", "Confirm")}
                              </button>
                              <button onClick={() => setDeleteArticleConfirm(null)} className="p-1 rounded-full hover:bg-accent transition-colors"><X className="w-4 h-4" /></button>
                            </div>
                          ) : (
                            <button onClick={() => setDeleteArticleConfirm(article.id)} className="p-2 rounded-full hover:bg-red-50 transition-colors" title={t("Ta bort", "Delete")}>
                    ...
                    <div className="text-center py-16 text-muted-foreground">
                      <p className="text-lg mb-4">{t("Inga artiklar ännu.", "No articles yet.")}</p>
                      <button onClick={handleNewArticle} className="inline-flex items-center gap-2 px-6 py-2.5 bg-[#c05746] text-white rounded-full text-sm font-semibold hover:bg-[#a8483b] transition-colors">
                        <Plus className="w-4 h-4" />{t("Skapa din första artikel", "Create your first article")}
                      </button>
                    </div>
                  )}
                </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full sm:w-64 px-4 py-2.5 rounded-lg bg-background border border-border/50 text-sm focus:outline-none focus:ring-2 focus:ring-[#c05746]/30"
                      />
                    </div>
                    <div>
                      <label className="block text-sm font-medium text-foreground mb-1">
                      ...
                      <textarea
                        ...
                        rows={6}
                        className="w-full px-4 py-2.5 rounded-lg bg-background border border-border/50 text-sm focus:outline-none focus:ring-2 focus:ring-[#c05746]/30 resize-y"
                    ...
                    <div className="flex items-center justify-between pt-2">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-2 px-8 py-2.5 bg-[#c05746] text-white rounded-full text-sm font-semibold hover:bg-[#a8483b] transition-colors shadow-md disabled:opacity-50"
                      >
                        {isDiarySaving ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />}
                        {t("Spara", "Save")}
                      </button>
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  {diaryData && diaryData.entries.length > 0 ? (
                    diaryData.entries.map((entry) => (
                      <div key={entry.id} className="bg-card rounded-xl border border-border/50 p-4 hover:shadow-sm transition-shadow">
                        <div className="flex items-start gap-4">
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-2">
                              <span className="text-sm font-semibold text-[#c05746]">
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            <button onClick={() => handleEditDiary(entry)} className="p-2 rounded-full hover:bg-accent transition-colors" title={t("Redigera", "Edit")}>
                              <Pencil className="w-4 h-4 text-muted-foreground" />
                            </button>
                            {deleteDiaryConfirm === entry.id ? (
                              <div className="flex items-center gap-1">
                                <button onClick={() => deleteDiaryMutation.mutate({ id: entry.id })} className="px-3 py-1 bg-red-500 text-white rounded-full text-xs font-medium hover:bg-red-600 transition-colors">
                                  {t("Bekräfta", "Confirm")}
                                </button>
                                <button onClick={() => setDeleteDiaryConfirm(null)} className="p-1 rounded-full hover:bg-accent transition-colors"><X className="w-4 h-4" /></button>
                              </div>
                            ) : (
                              <button onClick={() => setDeleteDiaryConfirm(entry.id)} className="p-2 rounded-full hover:bg-red-50 transition-colors" title={t("Ta bort", "Delete")}>
                                <Trash2 className="w-4 h-4 text-red-400" />
                              </button>
                            )}
                          </div>
                    ...
                    <div className="text-center py-16 text-muted-foreground">
                      <p className="text-lg mb-4">{t("Inga dagboksinlägg ännu.", "No diary entries yet.")}</p>
                      <button onClick={handleNewDiary} className="inline-flex items-center gap-2 px-6 py-2.5 bg-[#c05746] text-white rounded-full text-sm font-semibold hover:bg-[#a8483b] transition-colors">
                        <Plus className="w-4 h-4" />{t("Skriv ditt första inlägg", "Write your first entry")}
                      </button>
                    </div>
                  )}
                </div>
